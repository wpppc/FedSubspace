...
def main():
    ...
    cfg["output_dir"] = "outputs/fedless+dual"
    ...
    for r in range(start_round, rounds):
        ...
        for cid, dl_info in enumerate(client_dataloaders):
            ...
            # Instantiate client
            # lambda_reg = 0.0 for Ablation 2 (Dual Stream, Learnable Alpha, No Reg)
            ...
            if os.path.exists(local_state_path):
                ...
                # Initialize local theta to 0 (already done by wrapper init, but good to be sure)
                # But wrapper is shared, so we must reset it if it was modified by previous client
                # Actually, wrapper is shared, so we MUST reset local theta and alphas for each client if not loading!
                # Since we iterate sequentially, the previous client's state is in the model.
                # If no local state exists (Round 0), we should reset to 0.
        ...
        if cfg["eval"]["enabled"] and (r % cfg["eval"].get("eval_every", 1) == 0):
            ...
            if True:
                ...
                update_summary_csv(cfg["output_dir"], "FedLESS+Dual", r, client_scores)
