# main_math_elastic.py
import os, yaml, torch, json, csv, time, gc, random, math
...
from federated.elastic_components import ElasticFedSubspaceClient, ElasticFedSubspaceServer
...
def calculate_elastic_dim(dataset_size, d_max, lambda_val=1000, beta=0):
    """
    d_k = min(d_max, floor(lambda * log(size) + beta))
    """
    if dataset_size <= 0: return 100 # Minimum fallback
    val = lambda_val * math.log(dataset_size) + beta
    return max(100, min(d_max, int(val))) # Ensure at least 100 dims

def main(cfg_path="configs/fedsubspace_multi_domain.yaml"):
    cfg = yaml.safe_load(open(cfg_path,"r"))
    os.makedirs(cfg["output_dir"], exist_ok=True)

    # Force Dirichlet for Elastic Experiment
    cfg["data"]["partition_strategy"] = "dirichlet"
    print("Forcing Dirichlet partition for Elastic Subspace experiment.")
    ...
    # --- CSV Logging Setup ---
    csv_file = os.path.join(cfg["output_dir"], "experiment_results_elastic.csv")
    ...
    # We need to re-partition if it was uniform before, or just trust the user to clear it
    # For safety, we check if we need to re-partition
    ...
    # Server
    server = ElasticFedSubspaceServer(cfg["subspace"]["dim"])
    ...
    # Elastic Config
    # We want d_k to range from e.g. 1000 to 4096
    # log(5000) ~ 8.5, log(50000) ~ 10.8
    # 4096 = lambda * 10.8 + beta
    # 1000 = lambda * 8.5 + beta
    # 3096 = lambda * 2.3 => lambda ~ 1346
    # beta = 1000 - 1346 * 8.5 = -10441
    # Let's try lambda=1500, beta=-12000 (rough guess)
    # Or simpler: d_k = d_max * (log(size) / log(max_size))
    
    # Let's use a simpler heuristic for this experiment:
    # d_k = d_max * (size / max_size)^0.5 (square root scaling)
    # Or just use the log formula with arbitrary params
    lambda_val = 2000
    beta_val = -16000 # log(40000)=10.6 -> 21200 - 16000 = 5200 (capped at 4096)
                      # log(5000)=8.5 -> 17000 - 16000 = 1000
    ...
    for cid in range(cfg["data"]["num_clients"]):
        ...
        client = ElasticFedSubspaceClient(client_id=cid, model=shared_model, tokenizer=tokenizer, dataloader=dl,
        ...
        # Calculate Elastic Dim
        d_k = calculate_elastic_dim(len(ds), cfg["subspace"]["dim"], lambda_val, beta_val)
        client.set_active_dim(d_k)
        print(f"Client {cid}: Size={len(ds)}, Active Dim={d_k}")
    ...
    for r in range(rounds):
        ...
        thetas=[]; sizes=[]; active_dims=[]
        for cid in selected_cids:
            c = clients[cid]
            c.load_theta(server.global_theta)
            c.train()
            thetas.append(c.get_theta())
            sizes.append(len(c.dataloader.dataset))
            active_dims.append(c.active_dim)
            
            gc.collect()
            torch.cuda.empty_cache()
        
        new_theta = server.aggregate(thetas, sizes, active_dims)
        torch.save(new_theta, os.path.join(cfg["output_dir"], f"math_elastic_theta_round{r}.pt"))

        # Eval
        if cfg["eval"]["enabled"] and (r % cfg["eval"].get("eval_every",1) == 0):
            print(f"Decoding adapter for Math Elastic round {r}...")
                ...
                for eval_set in ["gsm8k", "math"]:
                    ...
                    if os.path.exists(eval_file):
                        ...
                        if examples:
                            ...
                            print(f"[Math Elastic] {eval_set} Result: {res_str}")
