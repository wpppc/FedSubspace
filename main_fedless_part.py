...
from federated.client_part import FedPartClient
from federated.server_part import FedPartServer
...
# Group Definitions
GROUP_0_CIDS = [1, 3, 6, 7] # Logic
GROUP_1_CIDS = [0, 2, 4, 5] # Semantic
...
def main():
    ...
    cfg["output_dir"] = "outputs/fedless_part"
    ...
    if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
        print(">> [Main] bfloat16 supported. Using bfloat16 for stability.")
    ...
    # 3. Initialize Subspace
    print("Initializing Subspace...")
    lora_shapes = extract_lora_shapes(base, cfg["lora"]["target_modules"], cfg["lora"]["r"])
    
    # Calculate full LoRA params
    full_lora_params = sum([s[0]*s[1] for s in lora_shapes.values()]) * 2 # A and B
    subspace_params = cfg["subspace"]["dim"]
    
    # Wrap Model
    shared_model = FedSubspaceModelWrapper(base, lora_shapes, subspace_params, seed=cfg["subspace"]["seed"], target_modules=cfg["lora"]["target_modules"])
    
    # Initialize Server
    server = FedPartServer(subspace_params)
    
    # Initialize Global Theta (Random)
    # Note: Wrapper initializes theta_s randomly. We sync server to it.
    ...
    # --- Resume Logic ---
    start_round = 0
    checkpoint_pattern = re.compile(r"theta_round(\d+).pt")
    checkpoints = []
    ...
    if checkpoints:
        ...
        checkpoint_path = os.path.join(cfg["output_dir"], f"theta_round{last_round}.pt")
    ...
    for r in range(start_round, rounds):
        ...
        for cid, dl_info in enumerate(client_dataloaders):
            ...
            # Determine Group ID
            if cid in GROUP_0_CIDS:
                group_id = 0
                group_name = "Logic"
            elif cid in GROUP_1_CIDS:
                group_id = 1
                group_name = "Semantic"
            else:
                print(f"Warning: Client {cid} not in any group. Skipping.")
                continue

            print(f"Training Client {cid} (Group {group_id}: {group_name})...")
            
            # Instantiate FedPartClient
            client = FedPartClient(client_id=cid, model=shared_model, tokenizer=tokenizer, dataloader=dl,
        ...
        # Aggregate (Using Partitioned Aggregation)
        print("Aggregating with Partition Strategy...")
        new_theta = server.aggregate_part(thetas, sizes, client_ids)
        ...
        if cfg["eval"]["enabled"] and (r % cfg["eval"].get("eval_every", 1) == 0):
            ...
            if True:
                ...
                update_summary_csv(cfg["output_dir"], "FedLESS-Part", r, client_scores)
